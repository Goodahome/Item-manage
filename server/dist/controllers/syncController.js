...
const itemDtoSchema = zod_1.z.object({
    ...
    quantityUnit: zod_1.z.string().optional().nullable(),
...
async function bootstrapSync(req, res) {
    ...
    const clientEvents = new Map(client.activityEvents
        .slice()
        .sort((a, b) => {
...
async function bootstrapSyncAck(req, res) {
    ...
    try {
        await prisma_1.prisma.$transaction(async (tx) => {
            ...
            isolationLevel: 'Serializable' // 使用可串行化隔离级别，确保最高一致性
        });
        // 返回成功结果和冲突信息
        return res.json((0, response_1.ok)({
            success: true,
        ...
        if (error.code === 'P2034' || error.message?.includes('timeout')) {
            return res.status(408).json((0, response_1.fail)({
                code: "SYNC_TIMEOUT",
